...
# --------- Caching ---------
@st.cache_data(show_spinner="Loading and parsing files...")
def cached_load_files(uploaded_files):
    return load_files(uploaded_files)

@st.cache_data(show_spinner="Chunking text...")
def cached_chunk_sections(documents):
    return chunk_sections(documents)

@st.cache_resource(show_spinner="Building FAISS index...")
def cached_build_faiss_index(chunks):
...
# --------- Retrieval Pipeline ---------
if uploaded_files:
    documents = cached_load_files(uploaded_files)
    ...
    if any(
        ...
        chunks = cached_chunk_sections(documents)

    index, chunk_texts = cached_build_faiss_index(chunks)
    ...
    if query:
        with st.spinner("Retrieving answer..."):
            ...
            if filename_question_match:
                keyword = filename_question_match.group(1).strip().replace(" ", "").lower()
                matched_docs = [doc for doc in documents if keyword in doc["filename"].lower().replace(" ", "")]
...
if uploaded_files:
    if st.button("Evaluate Model"):
        ...
        explanation_map = {
            "ROUGE-1": "Measures unigram overlap between predicted and actual answers.",
            "ROUGE-L": "Measures longest common subsequence for structure alignment.",
            "Cosine Similarity": "Semantic similarity between model and expected answers.",
